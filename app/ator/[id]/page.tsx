...
export async function generateMetadata({
  ...
  if (!person) return { title: 'Ator não encontrado' }
  return {
...
export default async function AtorPage({
  ...
  const [person, filmografiaResult, movieIds] = await Promise.all([
    fetchPersonDetail(id),
    fetchFilmografia(personId, 0, 'cast'),
    fetchPersonMovieIds(personId, 'cast'),
  ])
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-bg-dark via-bg-mid to-bg-dark flex flex-col">
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-20 text-center">
          <p className="text-muted-300 text-xl mb-6">Ator não encontrado.</p>
          <Link
            href="/filmes"
  ...
  const roleLabel = person.gender === 1 ? 'Atriz' : 'Ator'

  const { films: initialFilms, total: totalFilms } = filmografiaResult
      ...
      <main className="flex-1">
        ...
        <div className="container mx-auto px-4 max-w-7xl mt-10 md:mt-14 mb-8">
          ...
          {initialFilms.length > 0 && (
            <section className="mb-10">
              <h2 className="text-white font-semibold text-lg mb-4">Filmografia</h2>
              <FilmografiaGrid
